...
export const UI_DEBUG_SHOWCASE_ENABLED = true
...
const ShowcaseSwitcher = () => {
  ...
  const buttonWidth = mobile ? 360 : 222
  const buttonHeight = mobile ? 58 : 46

  return (
    <UiEntity
      uiTransform={{
        positionType: 'absolute',
        position: mobile ? { left: 18, bottom: 18 } : { right: 22, top: 22 },
        width: mobile ? 1564 : 246,
        padding: { top: 12, bottom: 12, left: 12, right: 12 },
        flexDirection: 'column',
        pointerFilter: 'block'
      }}
      uiBackground={{ color: { r: 0.07, g: 0.05, b: 0.03, a: 0.96 } }}
    >
      <Label
        value="UI Debug"
        ...
        uiTransform={{ width: '100%', height: mobile ? 28 : 24, margin: { bottom: 4 } }}
      />
      <Label
        value="Late-game seeded state"
        fontSize={mobile ? 18 : 15}
        color={C.textMute}
        textAlign="middle-center"
        uiTransform={{ width: '100%', height: mobile ? 22 : 18, margin: { bottom: 10 } }}
      />
      ...
      >
        {SHOWCASE_BUTTONS.map((button) => {
          const active = showcaseState.selected === button.key
          return (
            <UiEntity
              key={button.key}
              uiTransform={{
                width: buttonWidth,
                height: buttonHeight,
                margin: { right: mobile ? 0 : 0, bottom: 8 },
                alignItems: 'center',
                justifyContent: 'center'
              }}
              uiBackground={{
                color: active
                  ? { r: 0.23, g: 0.58, b: 0.24, a: 1 }
                  : { r: 0.18, g: 0.14, b: 0.09, a: 1 }
              }}
              onMouseDown={() => {
                playSound('buttonclick')
                switchPanel(button.key)
              }}
            >
              <Label
                value={button.label}
                fontSize={mobile ? 20 : 17}
                color={active ? C.textMain : C.header}
                textAlign="middle-center"
              />
            </UiEntity>
          )
      ...
      <UiEntity
        ...
        uiBackground={{ color: { r: 0.38, g: 0.24, b: 0.07, a: 1 } }}
        onMouseDown={() => {
          playSound('buttonclick')
          showcaseState.reseedVersion += 1
          showcaseState.seededKey = ''
